...
public class OrderServiceImpl implements OrderService{
    ...
    //values for resttemplate call
    @Value("${user.service}")
    String userinservice;

    @Value("${pizza-catalogue.service}")
    String pizza_catalogue_inservice;
...
//            user = this.restTemplate.getForObject("http://localhost:8083/user/users/" + order.getUid(), User.class);
         ...
         pizza = this.restTemplate.getForObject(pizza_catalogue_inservice+order.getPid(),Pizza.class);
         user = this.restTemplate.getForObject(userinservice+order.getUid(),User.class);
